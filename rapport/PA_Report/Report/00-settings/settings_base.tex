...
% Page style
% ----------
\usepackage[margin=2cm, left=2.5cm, right=2.5cm, twoside=true]{geometry}
\usepackage{fancyhdr}
...
% Modified titlespacing to align all title texts and body text to the same margin:
% The first argument (0pt) ensures the title block starts at the left margin.
% The asterisk (*) prevents indentation of the paragraph after the title.
\titlespacing*{\chapter}{0pt}{-10mm}{3mm} % Keep your vertical spacing
\titlespacing*{\section}{0pt}{3mm}{2mm}
\titlespacing*{\subsection}{0pt}{2mm}{2mm}
\titlespacing*{\subsubsection}{0pt}{2mm}{1mm}

% Modified \titleformat commands for left alignment of number and text.
% "[hang]" option ensures that if the title wraps, subsequent lines are indented
% to align with the start of the title text (after the number).
% \quad adds a standard space after the number/separator.
\titleformat{\chapter}[hang]
  {\Huge\bfseries}
  {\thechapter\hspace{12pt}\textcolor{gray80}{|}} % The number and separator
  {1em} % Separation between number/separator and title
  {\raggedright#1} % Title text, ragged right

\titleformat{\section}[hang]
  {\Large\bfseries}
  {\thesection} % The number
  {1em} % Separation
  {\raggedright#1} % Title text
