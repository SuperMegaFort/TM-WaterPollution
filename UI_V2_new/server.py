...
def import_and_predict():
    ...
    def write_exif(r):
        ...
        try:
            ...
            exif_dict["Exif"][piexif.ExifIFD.UserComment] = b"ASCII\x00\x00\x00" + json_tag.encode('ascii', 'ignore')
            
            # 5. Standard Windows (Tags XP) - Encodage UTF-16LE (Null-Terminated + Tuple)
            win_encoded = tuple((info_str + "\0").encode('utf-16le'))
            ...
            if item.get('status') != 'night':
                ...
                if os.path.exists(img_path) and img_path.lower().endswith(('.jpg', '.jpeg')):
                    try:
                        ...
                        tag_text = "POLLUE" if user_val == 1 else "PROPRE"
                        info_str = f"Status: {tag_text} | Confiance: {score_val:.2f}"
                        ...
                        # Standard Windows (Tags XP) - Encodage UTF-16LE (Null-Terminated + Tuple)
                        win_encoded = tuple((info_str + "\0").encode('utf-16le'))
                        exif_dict["0th"][40092] = win_encoded
                        exif_dict["0th"][40095] = win_encoded
